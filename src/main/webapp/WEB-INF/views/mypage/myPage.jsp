...
<div class="container">
	<table>
		<tr>
			...
			<td>
				<div id="d1">사진 <span class="sky">${pcnt }</span></div>
				<div class="po">
					
					<c:forEach var="po" items="${post }">
						
						<c:if test="${po.kind == '사진'}">
		...
		<tr>
			<td>
				<div id="d2">집들이 <span class="sky">${hcnt }</span></div>
				<div class="po">
					<c:forEach var="po" items="${post }">
						
						<c:if test="${po.kind == '집들이'}">
						
						<a href="${pageContext.request.contextPath}/community/detail?idx=${po.idx }">
							<img class="my_photo_img" src="${pageContext.request.contextPath}/resources/post/${po.idx }/${po.picture}">
						</a>
						</c:if>
					</c:forEach>
